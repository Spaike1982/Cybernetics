function [Xh,Xb,Yb,Kk,Phat,Pbar,t,cal] = SRUKFestim(g,x0,Y,dt,tfin)

% SRUKFestim.m: Square Root Unscented Kalmanfilter estimation: 
...
% Fuksjonsrutine
%
%   [Xh,Xb,Yb,Kk,Phat,Pbar,t] = SRUKFestim(g,x0,Y,dt,tfin)
%
% Andre funksjoner som kreves internt
%
%   physcheck.m - Sjekke at ikke fysiske grenser blir brutt
%   SRUKF.m - Beregne estimater for hvert tidsskritt
% 
% Innganger:
...
for k = 1:nSim
    p = k/nSim*100;
    disp(['Prosent ferdig av SRUKF : ',num2str(p)]);
    ...
    % Kalmanfilter
    tic;
    [xh,xb,rhob,K,Sx] = SRUKF(g,xh,rho,wm,vm,Sx,Sw,Sv,W,gamma,h);
    cal(k) = toc;
